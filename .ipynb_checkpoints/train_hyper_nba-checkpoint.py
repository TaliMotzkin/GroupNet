...
def validate(validation_loader):
    ...
    with torch.no_grad():
        for data in validation_loader:
            total_loss, loss_pred, loss_recover, loss_kl, loss_diverse, my_loss, _, _, _, _ , _, _= model(data, 0, None)
...
def plot_variance(T, N, variance_per_agent):
    ...
    print("variance for agnets in time T:", variance_per_agent[:, 0, :])
...
def plot_xgb(evals_result, epoch, model):
    ...
    # shap_values = explainer(X_test) #complicate to obtain.....
    # plt.figure(figsize=(12, 8))  # Set figure size
    # shap.summary_plot(shap_values, X_test, show=False) 
    # plt.savefig(f"shap_summary_{epoch}_tight", dpi=300, bbox_inches='tight')
    # plt.savefig(f"shap_summary_{epoch}")
...
def train(train_loader, epoch, batch_size, device):
    ...
    avg_loss = 0
    all_loss = 0
    trajectory_wins = {agent: torch.zeros(20, dtype=torch.int64, device =device) for agent in range(8)}
    model_xgb = None
    for data in train_loader:
        total_loss, loss_pred, loss_recover, loss_kl, loss_diverse, my_loss, outputs_logits, predictions, actual_probs, true_indices, evals_result, model_xgb = model(data, iter_num, model_xgb)
        ...
        for agent in range(N):
            trajectory_wins[agent] += true_indices[:, agent, :].sum(dim=0).to(torch.int64)
    ...
    trajectory_wins_np = {agent: trajectory_wins[agent].cpu().detach().numpy() for agent in range(N)}
    plot_variance(T, N, variance_per_agent.cpu().detach().numpy() )
    validate_loss = validate(validation_loader)
    scheduler.step()
    model.step_annealer()
    avg_loss = all_loss / iter_num
    plot_agent_trajectory_histogram(3, trajectory_wins_np, S, epoch)

    return avg_loss, validate_loss, diversity_score.cpu().detach().numpy() 

def plot_agent_trajectory_histogram(agent_id, trajectory_wins, S, epoch):
    ...
    plt.xticks(range(S))
    plt.savefig(f"agent_{agent_id}_traj_histogram_epoch_{epoch}")
    plt.show()
    plt.close()
...
def ploting_diversity(epochs, N,avg_diversity_np):
    avg_diversity_np = np.array(avg_diversity) 
    selected_epochs = np.arange(0, epochs, 5)  # Creates [0, 5, 10, 15, ...]
    width = 0.08

    fig, ax = plt.subplots(figsize=(12, 6))

    for agent in range(N):
        x_pos = selected_epochs + (agent - N / 2) * (2 * width)  # Spread bars for clarity
        ax.bar(x_pos, avg_diversity_np[selected_epochs, agent, 0], width=width, label=f'Agent {agent} X')
        ax.bar(x_pos + width, avg_diversity_np[selected_epochs, agent, 1], width=width, label=f'Agent {agent} Y')

    ax.set_xlabel("Epochs")
    ax.set_ylabel("Diversity Score Variance")
    ax.set_title("Diversity Score Variance per Agent (Every 5 Epochs)")
    ax.legend(ncol=4, fontsize=8, loc='upper right')
    plt.xticks(selected_epochs)  # Show only selected epochs
