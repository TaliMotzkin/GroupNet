...
class GroupNet(nn.Module):
    ...
    def run_xgb(self, diverse_pred_traj,future_traj, iter_num, xgb_model):
        ...
        if iter_num == 0 :
            new_model=  xgb.train(params = params, dtrain=dtrain, evals = [(dtrain, "train")], evals_result=evals_result, verbose_eval = False)
        else:
            new_model= xgb.train (params = params, dtrain=dtrain, evals = [(dtrain, "train")],evals_result=evals_result, xgb_model=xgb_model, verbose_eval = False)
            
       
        return  evals_result, new_model
        

    def forward(self,data, iter_num, model_xgb):
        device = self.device
        ...
        results_xgb, model_xgb = self.run_xgb(diverse_pred_traj,future_traj, iter_num, model_xgb)

        total_loss = loss_pred + loss_recover + loss_kl + loss_diverse + loss_true


        return total_loss, loss_pred.item(), loss_recover.item(), loss_kl.item(), loss_diverse.item(), loss_true.item(), outputs, diverse_pred_traj, soft_targets, true_indices ,results_xgb, model_xgb
